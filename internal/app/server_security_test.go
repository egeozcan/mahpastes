...
func TestSPAHandlerGatesShellButServesPublicAssets(t *testing.T) {
	db := newServerTestDB(t)
	app := &App{db: db}
	manager := NewAPIManager(app, t.TempDir())
	if manager.signingKey == nil {
		t.Fatal("expected signing key to be initialized")
	}

	assets := fstest.MapFS{
		"index.html":      {Data: []byte("<head></head><body>app</body>")},
		"login.html":      {Data: []byte("<html>login</html>")},
		"dist/output.css": {Data: []byte(".x{}")},
	}
	h := newSPAHandler(assets, manager)

	// Unauthenticated: the SPA shell (root, /index.html, unknown deep links)
	// must redirect to the login page — never serve the shell.
	for _, p := range []string{"/", "/index.html", "/unknown/deep/link"} {
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, p, nil))
		if rec.Code != http.StatusFound {
			t.Fatalf("GET %s unauth: expected 302, got %d", p, rec.Code)
		}
		if loc := rec.Header().Get("Location"); loc != "/login.html" {
			t.Fatalf("GET %s unauth: expected redirect to /login.html, got %q", p, loc)
		}
	}

	// Unauthenticated: public static assets are served so the login page renders.
	for _, p := range []string{"/login.html", "/dist/output.css"} {
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, p, nil))
		if rec.Code != http.StatusOK {
			t.Fatalf("GET %s unauth: expected 200, got %d", p, rec.Code)
		}
	}

	// Authenticated: shell is served with the server-mode flag injected.
	...
	for _, p := range []string{"/", "/index.html"} {
		...
		h.ServeHTTP(rec, req)
		if rec.Code != http.StatusOK {
			t.Fatalf("GET %s authed: expected 200, got %d", p, rec.Code)
		}
		if !strings.Contains(rec.Body.String(), "window.mahpastesMode = 'server'") {
			t.Fatalf("GET %s authed: expected injected server-mode script, body=%q", p, rec.Body.String())
		}
	}
}
