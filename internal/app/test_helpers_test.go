...
func setTestEventSink(t *testing.T, app *App, sink func(name string, data ...interface{})) {
	t.Helper()
	bridge, ok := app.bridge.(*wailsbridge.Bridge)
	if !ok {
		t.Fatalf("test app bridge has unexpected type %T", app.bridge)
	}
	bridge.SetTestEventSink(sink)
}
