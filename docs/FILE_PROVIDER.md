...
On macOS, `make build` (and therefore `make install`) builds this integration
when signing is configured; users enable it in Settings. The native variant requires macOS 13
or later. Direct `wails build`, binding generation, `mp`
and `mahpastesd` do not require Swift, Xcode or Apple signing credentials.

## Build and install

The native build needs Apple Command Line Tools (including Swift and the macOS
SDK), Python 3, the Go toolchain specified by `go.mod`, Node, and Wails v2.12.0.
Full Xcode and XcodeGen are not required. The script compiles both extension
architectures with `swiftc`, combines them with `lipo`, and packages the `.appex`
bundle directly. Install Command Line Tools with `xcode-select --install` if
needed.

Xcode and XcodeGen are used only for the XCTest suite in CI. The standalone
native lifecycle checks also run with Command Line Tools.
...
```sh
make build # On macOS; falls back to the ordinary app without signing configuration.
```
...
### Sign a CI artifact without compiling locally
